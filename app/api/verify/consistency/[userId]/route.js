...
export const GET = async (req, { params }) => {
  try {
    ...
    const consistencyGoal = foundUser.goals[0]; // Assuming there's only one consistency goal

    // Check if the weekLog array is not empty
    if (consistencyGoal.consistency.weekLog.length > 0) {
      // Get the latest entry in the weekLog (which is the first entry)
      const latestUpdateDate = consistency.latestUpdateDate;
      const currentDate = new Date();

      // Check if the latestUpdateDate is on the same day as the current day
      if (
        currentDate.getUTCFullYear() ===
          latestUpdateDate.date.getUTCFullYear() &&
        currentDate.getUTCMonth() === latestUpdateDate.date.getUTCMonth() &&
        currentDate.getUTCDate() === latestUpdateDate.date.getUTCDate()
      ) {
